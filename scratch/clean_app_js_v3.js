...
langs.forEach(lang => {
    ...
    if (startIdx === -1) {
        console.error(`Lang block not found: ${lang}`);
        return;
    }
    
    // Find limit of this language block uniquely
    const nextLangs = [
        '\n        en: {',
        '\n        ar: {',
        '\n        ka: {',
    ...
    // Split block content into lines and filter out lines containing projectKeys
    ...
    // Find the last closing brace '}' in the cleaned block content
    let lastBraceIdx = cleanBlock.lastIndexOf('}');
    if (lastBraceIdx === -1) {
        console.error(`No closing brace found for ${lang}`);
        return;
    }
    ...
    // Reconstruct block by inserting before the last brace
    let newBlockContent = cleanBlock.substring(0, lastBraceIdx) + injection + '\n        }';
    
    // Check if the original block ended with a comma
    if (cleanBlock.substring(lastBraceIdx).includes(',')) {
        newBlockContent += ',';
    }
    
    content = content.substring(0, startIdx) + newBlockContent + content.substring(endIdx);
    console.log(`Cleaned and updated translations for ${lang}`);
});

fs.writeFileSync(jsPath, content, 'utf8');
console.log("Success! Clean project translations written to js/app.js");
