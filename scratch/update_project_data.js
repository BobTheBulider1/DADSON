...
for (const [lang, keys] of Object.entries(translations)) {
    const startIdx = appJs.indexOf(`${lang}: {`);
    if (startIdx === -1) {
        console.error(`Language block not found: ${lang}`);
        continue;
    }
    
    // Find limits of this language block to restrict replacements
    const nextLangs = ['en:', 'ar:', 'ka:', 'ru:', 'fr:', 'it:', '};'];
    let endIdx = appJs.length;
    for (const nextLang of nextLangs) {
        const idx = appJs.indexOf(nextLang, startIdx + 10);
        if (idx !== -1 && idx < endIdx) {
            endIdx = idx;
        }
    }
    
    let blockContent = appJs.substring(startIdx, endIdx);
    
    for (const [key, newVal] of Object.entries(keys)) {
        // Regex to match the key and value inside quotes, handling both single/double quotes and backslashes
        const regex = new RegExp(`(${key}\\s*:\\s*)(['"])([\\s\\S]*?)(['"])(,?)`, 'g');
        
        if (regex.test(blockContent)) {
            blockContent = blockContent.replace(regex, (match, prefix, q1, oldVal, q2, comma) => {
                const escapedVal = newVal.replace(/"/g, '\\"');
                console.log(`Replacing ${lang} -> ${key}`);
                return `${prefix}"${escapedVal}"${comma}`;
            });
        } else {
            console.warn(`Key not found in ${lang}: ${key}. We will append it.`);
            // Append right after the "lang: {" block header
            const insertIdx = blockContent.indexOf('{') + 1;
            const escapedVal = newVal.replace(/"/g, '\\"');
            blockContent = blockContent.slice(0, insertIdx) + `\n            ${key}: "${escapedVal}",` + blockContent.slice(insertIdx);
        }
    }
    
    // Put modified block back
    appJs = appJs.substring(0, startIdx) + blockContent + appJs.substring(endIdx);
}

fs.writeFileSync(jsPath, appJs, 'utf8');
console.log("Translations updated in js/app.js successfully!");
